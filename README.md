## Модуль для BRM системы мебельной фабрики

### Описание.
Данный модуль представляет собой один из компонентов BPM системы для мебельной фабрики. Его основная функция заключается в печати списка деталей изделия, полученного из программы Bcad в формате CSV. Изначально он был разработан на базе React, входя в состав BPM системы. Позднее, с целью оптимизации и обеспечения максимальной совместимости, модуль был переписан на нативный JavaScript, используя подход AJAX.
...
1.  **Импорт данных**: Модуль позволяет пользователям BPM системы загружать список деталей в формате CSV, полученный из программы Bcad. Для обработки CSV данных используется npm пакет `iconv-lite`, что обеспечивает корректное чтение данных с различными кодировками.
